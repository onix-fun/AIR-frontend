...
const defaultAccountApiUrl = import.meta.env.DEV
  ? "http://localhost:8089/api"
  : trustedBaseDomain
    ? `https://account.${trustedBaseDomain}/api`
    : "/api";

export const APP_API_BASE_URL = (import.meta.env.VITE_APP_API_URL || import.meta.env.VITE_API_URL || "/api").replace(
  /\/$/,
  "",
);
export const ACCOUNT_API_BASE_URL = (import.meta.env.VITE_ACCOUNT_API_URL || defaultAccountApiUrl).replace(/\/$/, "");
export const DOMAIN_BASE_URL = `${APP_API_BASE_URL}/domain`;
export const ANALYTICS_BASE_URL = `${APP_API_BASE_URL}/analytics`;
export const PROFILE_BASE_URL = ACCOUNT_API_BASE_URL;
