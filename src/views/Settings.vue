...
<template>
  <div class="settings-page">
    <span v-if="profileMessage" class="status-badge" :class="messageTone" style="margin-bottom: 1rem">{{ profileMessage }}</span>
    ...
    <section class="settings-section">
      <div class="profile-head">
        ...
        <input id="avatar-input" class="visually-hidden" type="file" accept="image/jpeg,image/png,image/webp" @change="onAvatarChange" />
    ...
    <section class="settings-section">
      ...
      <div class="session-list">
        ...
        <div v-for="session in authStore.sessions" :key="session.id" class="session-row" :class="{ 'is-current': session.isCurrent }">
          <div class="session-info">
            <i class="pi pi-desktop"></i>
            <div>
              <div class="session-device">{{ sessionDevice(session) }}</div>
              <span class="session-meta">{{ session.ipAddress || "Unknown" }} &middot; {{ formatDate(session.lastUsedAt) }}</span>
            </div>
          </div>
