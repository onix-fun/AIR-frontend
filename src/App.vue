...
const bootstrap = async () => {
  ...
  try {
    await Promise.all([
      ...
      deviceStore.fetchConsumers(),
      analyticsStore.checkHealth(),
    ]);
    await Promise.all([
...
const resendRegistrationCode = async () => {
  authMessage.value = '';
  try {
    const response = await authStore.resendRegistrationCode(pendingRegistrationEmail.value || registerForm.value.email);
    pendingRegistrationEmail.value = response.email;
...
<template>
  <section v-if="!authStore.isAuthenticated || authMode === 'name'" class="login-screen">
    <div class="login-panel">
      ...
      <h1>{{ authMode === 'register' || authMode === 'confirm' || authMode === 'name' ? 'Create your Sparrow Account' : 'Sparrow' }}</h1>
      ...
      <div v-if="authMode === 'register' || authMode === 'confirm' || authMode === 'name'" class="auth-steps">
        <span :class="{ active: authMode === 'register', done: authMode === 'confirm' || authMode === 'name' }">Account</span>
      ...
      <div v-if="authMode !== 'name'" class="auth-tabs">
        <button type="button" :class="{ active: authMode === 'login' }" @click="authMode = 'login'">Sign in</button>
      ...
      <form v-else-if="authMode === 'confirm'" class="login-form" @submit.prevent="confirmRegistration">
        ...
        <label>
          Verification code
          <input v-model="registrationCode" inputmode="numeric" pattern="[0-9]{6}" autocomplete="one-time-code" required />
        </label>
        <button class="btn btn-primary" type="submit" :disabled="authStore.isLoading">
        ...
        <div class="auth-inline-actions">
          <button class="link-button" type="button" :disabled="authStore.isLoading" @click="resendRegistrationCode">
            Resend code
          </button>
      ...
      <form v-else class="login-form" @submit.prevent="submitResetPassword">
        ...
        <label>
          Reset code
          <input v-model="resetCode" inputmode="numeric" pattern="[0-9]{6}" autocomplete="one-time-code" required />
        </label>
        <label>
          New password
